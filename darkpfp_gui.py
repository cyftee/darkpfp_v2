...
# -*- coding: utf-8 -*-
"""
darkpfp_gui.py — простий GUI для стилю darkpfp із покращеними контурами та збереженням пресетів.

Фішки:
- Режими контурів: Fusion (реком.), Canny, HED (DL; опційно, якщо є модель)
- Foreground підсилення: Saliency (opencv-contrib) + опційно Segmentation (U2Net/GrabCut)
- Thinning (скелетизація): ximgproc або морф-скелет
- Апскейл 1×/2×/4× до стилізації, квадрат: пад або розтяг
- Стилі виводу: Neon (darkpfp) / Line Art (білі контури на чорному)
- Пресети зберігаються/видаляються через GUI і живуть у INI-файлі поряд зі скриптом
- Пам'ятає останні шляхи відкриття/збереження + останній обраний пресет
...
(Опційно для HED)
  Покладіть у папку скрипта:
    - hed_pretrained_bsds.caffemodel
    - deploy.prototxt (або hed.prototxt)

(Опційно для Segmentation Boost)
  Покладіть у папку скрипта: u2net.onnx (або u2netp.onnx)
...
def load_user_presets(cfg):
    ...
    for sect in cfg.sections():
        if sect.startswith("preset:"):
            ...
            p = {}
            getf = cfg.getfloat; geti = cfg.getint; gets = cfg.get; getb = cfg.getboolean
            def g(key, default, conv=lambda x:x):
                try:
                    val = gets(sect, key, fallback=None)
...
def add_vignette(img, amount=0.35):
    ...
    dist = np.sqrt((X-cx)**2 + (Y-cx)**2); maxd = np.sqrt(cx**2 + cy**2)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("darkpfp — покращені контури + пресети (INI)")
        ...
        row1.addWidget(self.btn_open); row1.addWidget(self.btn_save)
        lyb.addLayout(row1)
        ...
        for g in (grp_edge_thresh, grp_clahe_clip, grp_clahe_grid, grp_ms_levels, grp_chroma_w, self.sp_log_sigma, self.sp_morph_d, self.sp_morph_e, self.sp_median_ks):
            pass  # placeholders to ensure variables exist
        ...
        self.lbl_in.setMinimumSize(500,380); self.lbl_in.setFrameShape(QtWidgets.QFrame.Box); right.addWidget(self.lbl_in, 1)
        self.lbl_out = QtWidgets.QLabel("Предпросмотр"); self.lbl_out.setAlignment(QtCore.Qt.AlignCenter)
        self.lbl_out.setMinimumSize(500,380); self.lbl_out.setFrameShape(QtWidgets.QFrame.Box); right.addWidget(self.lbl_out, 1)
